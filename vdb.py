...
from embedders.base import EmbedderConfig
...
class WrapperClass:
    def __init__(self, name: str, embedder_config: EmbedderConfig):
        self.name = name
        self.embedding_fn = modal.Cls.from_name(embedder_config.app_name, embedder_config.name)(**embedder_config.params).embed.remote
...
def main():
    ...
    #embedding_fn = SentenceTransformersEmbedder
    embeddings: list[np.array] = list(embedding_fn.embed.map([pokemon["description"] for pokemon in first_10_pokemon]))
    remote_db = WrapperClass(name="pokemon_two_db", embedder_config=EmbedderConfig(name="SentenceTransformersEmbedder", dims=384, params={"model_name": "all-MiniLM-L6-v2"}))
